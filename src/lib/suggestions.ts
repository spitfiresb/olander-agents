...
//
// Aggregation questions (totals/counts/rankings) are now answerable via the
// `aggregate` tool, but they're kept OUT of these demo chips: aggregate scans
// are bounded and can return partial results, which doesn't make for a crisp
// one-click demo. The eval exercises the aggregate path via ACCURACY_PROBES in
// scripts/eval.ts instead. No cost/margin prompts here — those columns require
// the opt-in `pricing` scope and are redacted for non-admins.
